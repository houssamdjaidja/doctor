...
async function main() {
  ...
  // Appointments
  const apts = [
    { pi: patientIds[0], fn: 'Marie', ln: 'Dupont', ph: '0612345678', em: 'marie.dupont@email.com', dt: '2024-01-20', ts: '10:00', mf: 'Consultation gأ©nأ©rale', ns: '', st: 'confirmed' },
    { pi: patientIds[0], fn: 'Marie', ln: 'Dupont', ph: '0612345678', em: 'marie.dupont@email.com', dt: '2024-01-27', ts: '14:30', mf: 'Suivi mأ©dical', ns: '', st: 'pending' },
    { pi: patientIds[0], fn: 'Marie', ln: 'Dupont', ph: '0612345678', em: 'marie.dupont@email.com', dt: '2024-01-10', ts: '09:00', mf: 'Consultation gأ©nأ©rale', ns: 'Prescription: Doliprane 1000mg', st: 'completed' },
    { pi: patientIds[0], fn: 'Marie', ln: 'Dupont', ph: '0612345678', em: 'marie.dupont@email.com', dt: '2023-12-28', ts: '11:00', mf: 'Contrأ´le de santأ©', ns: 'Bilan sanguin normal', st: 'completed' },
    { pi: patientIds[1], fn: 'Pierre', ln: 'Lambert', ph: '0623456789', em: 'pierre.lambert@email.com', dt: '2024-01-20', ts: '10:00', mf: 'Suivi mأ©dical', ns: '', st: 'in-progress' },
    ...
    { pi: patientIds[3], fn: 'Jean', ln: 'Durand', ph: '0645678901', em: 'jean.durand@email.com', dt: '2024-01-20', ts: '14:00', mf: 'Contrأ´le de santأ©', ns: '', st: 'pending' },
    { pi: patientIds[4], fn: 'Claire', ln: 'Moreau', ph: '0656789012', em: 'claire.moreau@email.com', dt: '2024-01-20', ts: '15:00', mf: 'Suivi mأ©dical', ns: '', st: 'pending' },
  ];
  for (const a of apts) {
    await run(
  ...
  // FAQ items
  const faqs = [
    { ci: fc1.rows[0].id, q: 'Comment prendre rendez-vous ?', a: 'Vous pouvez prendre rendez-vous de plusieurs faأ§ons : en ligne via notre formulaire de prise de rendez-vous, par tأ©lأ©phone au +213 21 23 45 67, ou directement au cabinet.', so: 1 },
    { ci: fc1.rows[0].id, q: 'Quels sont les dأ©lais pour obtenir un rendez-vous ?', a: 'Nous nous efforأ§ons de vous proposer un rendez-vous dans les meilleurs dأ©lais. En gأ©nأ©ral, un rendez-vous est disponible sous 24 أ  48 heures pour les consultations non urgentes.', so: 2 },
    { ci: fc1.rows[0].id, q: 'Puis-je modifier ou annuler mon rendez-vous ?', a: "Oui, vous pouvez modifier ou annuler votre rendez-vous jusqu'أ  24 heures avant l'heure prأ©vue sans frais.", so: 3 },
    { ci: fc1.rows[0].id, q: 'Dois-je arriver en avance pour mon rendez-vous ?', a: 'Oui, nous vous recommandons d\'arriver 10 أ  15 minutes avant votre rendez-vous pour complأ©ter les formalitأ©s administratives si nأ©cessaire.', so: 4 },
    { ci: fc2.rows[0].id, q: "Quels sont les horaires d'ouverture du cabinet ?", a: 'Le cabinet est ouvert du dimanche au jeudi de 8h00 أ  18h00, et le samedi de 9h00 أ  12h00. Nous sommes fermأ©s le vendredi et les jours fأ©riأ©s.', so: 1 },
    { ci: fc2.rows[0].id, q: 'Quels documents dois-je apporter pour ma consultation ?', a: "Pensez أ  apporter votre carte Chifa, votre carte de complأ©mentaire santأ©, une piأ¨ce d'identitأ©, et tous les documents mأ©dicaux pertinents.", so: 2 },
    { ci: fc2.rows[0].id, q: 'Comment accأ©der au cabinet ?', a: 'Le cabinet est situأ© au 24 Rue Didouche Mourad, 16000 Alger. Il est accessible en mأ©tro, tramway, bus et taxi.', so: 3 },
    { ci: fc2.rows[0].id, q: 'Y a-t-il un parking أ  proximitأ© ?', a: 'Oui, un parking public se trouve أ  2 minutes أ  pied du cabinet. Il est payant.', so: 4 },
    { ci: fc3.rows[0].id, q: 'Quels moyens de paiement sont acceptأ©s ?', a: 'Nous acceptons les paiements par carte bancaire, chأ¨que, espأ¨ces et virement. Le paiement sans contact est disponible.', so: 1 },
    { ci: fc3.rows[0].id, q: 'Acceptez-vous la carte Chifa et les complأ©mentaires santأ© ?', a: 'Oui, nous acceptons la carte Chifa et les cartes de complأ©mentaire santأ© pour le tiers payant.', so: 2 },
    { ci: fc3.rows[0].id, q: 'Quels sont les tarifs des consultations ?', a: 'Les tarifs varient selon le type de consultation : 2 500 DA pour une consultation gأ©nأ©rale, 3 000 DA pour un suivi, 5 000 DA pour un bilan de santأ© complet.', so: 3 },
    { ci: fc3.rows[0].id, q: 'Proposez-vous des facilitأ©s de paiement ?', a: 'Pour les patients en difficultأ© financiأ¨re, nous pouvons أ©tudier des solutions de paiement adaptأ©es.', so: 4 },
  ];
  for (const f of faqs) {
  ...
  const pm1 = await run(
    ...
    patientIds[0], 'Question sur mon traitement', 'Bonjour docteur, j\'ai une question concernant mon traitement. Est-ce que je peux prendre du Doliprane avec mon mأ©dicament actuel ?',
    'Bonjour Marie, il n\'y a pas de contre-indication connue. Vous pouvez prendre du Doliprane 1000mg si nأ©cessaire, sans dأ©passer 3 comprimأ©s par jour.'
  );
  await run(
    'INSERT INTO patient_messages (patient_id, subject, message) VALUES (?, ?, ?)',
    patientIds[0], 'Rأ©sultats d\'analyse', 'Bonjour, j\'ai reأ§u mes rأ©sultats d\'analyse. Pouvez-vous me dire si tout va bien ?'
  );
  await run(
    'INSERT INTO patient_messages (patient_id, subject, message, reply, reply_at, read_by_admin, read_by_patient) VALUES (?, ?, ?, ?, NOW(), TRUE, TRUE) RETURNING id',
    patientIds[1], 'Demande de renouvellement', 'Bonjour Dr. Djaidja, pourriez-vous renouveler mon ordonnance ? Merci.',
    'Bonjour Pierre, votre ordonnance a أ©tأ© renouvelأ©e. Vous pouvez passer au cabinet pour la rأ©cupأ©rer.'
  );
  ...
  await run('INSERT INTO patient_documents (patient_id, name, file_type, file_data, file_size) VALUES (?, ?, ?, ?, ?)',
    patientIds[0], 'Rأ©sultats analyse sang.pdf', 'application/pdf', fakePdf, 342);
